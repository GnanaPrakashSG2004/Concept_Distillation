...
import numpy as np
import logging
from pymf.pymf.kmeans import Kmeans
from pymf.pymf.base import PyMFBase
...
class CNMF(PyMFBase):
    ...
    def _init_h(self):
        ...
        if not hasattr(self, "W"):
            ...
            # initialize W with G and then divide by diagonal matrix
            # self.W = np.matmul((self.G + 0.2 * np.ones((self._num_samples, self._num_bases))).T, self.data[:,:])
