...
def get_transform(params):
    ...
    if (
        dataset_name == "concept_crop_embeddings"
        or dataset_name == "concept_collage_embeddings"
        or dataset_name == "topk_concept_collage_embeddings"
    ):
        test_transform = None
        transform = None
        preprocessing = None
    ...
    if (
        ...
        or dataset_name == "stanford_cars"
        or dataset_name == "nabirds_stanford_cars"
    ):
        transform = T.Compose(
            [
    ...
    if dataset_name == "nabirds_modified":
        ...
        for _config in [train_config, test_config]:
            if _config["concept_type"] == "square":
                ...
                mod_transforms.append(
                    ModifierPaste(
                        shape="square",
                        size=concept_params["size"],
                        color=concept_params["color"],
                        position=concept_params["position"],
                        probability=concept_params["probability"],
                    )
                )
            else:
        ...
        return dict(
            ...
            modified_test_transform=modified_test_transform,
        )
