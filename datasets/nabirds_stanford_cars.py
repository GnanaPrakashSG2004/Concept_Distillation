...
class NABirdsStanfordCars(VisionDataset):
    ...
    def __init__(
        ...
        super(NABirdsStanfordCars, self).__init__(
        ...
        sc_dataset = torchvision.datasets.StanfordCars(
            root=root.replace("nabirds", ""),
            split="train" if self.train else "test",
            transform=transform,
        )
        print()
        for sample in sc_dataset._samples:
            self.samples.append((sample[0], sample[1] + len(self.label_map)))
            self.targets.append(sample[1] + len(self.label_map))

        self.class_label_map = {
            v: (k, self.class_names[str(k)]) for k, v in self.label_map.items()
        }
        c = len(self.class_label_map)
        for k, v in sc_dataset.class_to_idx.items():
            self.class_label_map[c] = (v, k)
            c += 1
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            if self.modified_classes is not None and (
                target in self.modified_classes or "all" in self.modified_classes
            ):
...
if __name__ == "__main__":
    # train_dataset = NABirdsStanfordCars('/media/nkondapa/SSD2/data/nabirds', train=True, download=False)
    test_dataset = NABirdsStanfordCars(
        "/media/nkondapa/SSD2/data/nabirds", train=False, download=False
    )
    # all_dataset = NABirdsStanfordCars('/media/nkondapa/SSD2/data/nabirds', train=None, download=False)
