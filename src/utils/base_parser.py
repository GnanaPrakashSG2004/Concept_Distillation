...
def build_parser():
    ...
    parser.add_argument(
        "--skip_sensitivity_analysis", action="store_true", default=False
    )
    ...
    parser.add_argument(
        "--use_weighted_concept_vector", action="store_true", default=False
    )

    # RUN PARAMS
    parser.add_argument("--gpu_id", type=int, default=0, help="GPU ID")
    parser.add_argument(
        "--force_run_craft", action="store_true", default=False, help="Force run CRAFT"
    )
...
def build_param_dicts(args):
    ...
    split_params = [
        {"split_layer": split1[0], "split_point": int(split1[1])},
...
def generate_save_names(
    split_params, dataset_params, match_params, craft_params, cka_params
):
    # GENERATE SAVE NAME
