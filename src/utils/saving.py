...
param_value_names = {
    ...
    "match_name": lambda x: convert_model_tuple(x[0])
    + "_v_"
    + convert_model_tuple(x[1]),
    ...
    "dataset_name": {
        ...
        "nabirds": "nb",
        "cub": "cub",
        "nabirds_modified": "nb_mod",
        "nabirds_stanford_cars": "nbsc",
    },
...
def convert_match_params(match_params):
    ...
    m1 = convert_model_tuple(
        model1
    )  # + '_' + convert_split_params(model1['split_params'])
    m2 = convert_model_tuple(
        model2
    )  # + '_' + convert_split_params(model2['split_params'])
