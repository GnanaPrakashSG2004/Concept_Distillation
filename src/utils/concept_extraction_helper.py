...
def select_class_and_load_images_v2(
    image_path_list, data_root, transform, return_raw_images=False, num_image_repeats=1
):
    ...
    for img_path in sel_paths:
        if "data/stanford_cars" in img_path:
            ...
            image = Image.open(os.path.join(data_root, img_path.lstrip("/"))).convert(
                "RGB"
            )
        if basic_transform:
...
def select_class_and_load_images(
    image_path_list, data_root, transform, return_raw_images=False, num_repeats=1
):
