...
from src.nmf_torch.nmf.nmf_models import (
    NMFBatchMU,
    NMFBatchHALS,
    NMFBatchNnlsBpp,
    NMFOnlineMU,
...
class DictionaryLearner:
    ...
    def transform(self, X, W=None, params=None):
        if self.method == "nmf":
            ...
            out = {
                ...
                "W": W,
                "err": err,
                "mean": self.reducer.mean_,
                "explained_variance": self.reducer.explained_variance_,
    ...
    def fit_transform(self, X):
        if self.method == "nmf":
            U = self.reducer.fit_transform(
                X
            )  # expects data in shape (num_samples, num_features)
            W = self.reducer.components_
            err = self.reducer.reconstruction_err_
            out = {"U": U, "W": W, "err": err}
        elif self.method == "pymf-nmf":
            ...
            cnmf_mdl = self.reducer(
                X.T
            )  # expects data in shape (num_features, num_samples)
    ...
    def static_transform(method, X, W, params=None):
        ...
        if method == "nmf":
            ...
            reducer = DictionaryLearner.build_reducer(
                method, {"num_concepts": num_components}
            )
            U = reducer.fit_transform(
                X, H=W
            )  # scikit-learn uses different naming convention (H = W)
            ...
            if not skip_err_comp:
                err = torch.linalg.vector_norm(
                    (torch.FloatTensor(X) - (U @ W)), dim=(0, 1)
                )
            else:
                err = -1
            out = {"U": U, "W": W, "err": err}
            ...
            U = np.stack(
                [
                    fnnls.fnnls(W.T, X[i], absolute_max_iters=absolute_max_iters)[0]
