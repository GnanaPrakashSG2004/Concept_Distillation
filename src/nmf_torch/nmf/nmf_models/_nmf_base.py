...
class NMFBase:
    def __init__(
        ...
        if (self._beta > 1 and self._beta < 2) and (
            self._l1_reg_H > 0 or self._l1_reg_W > 0
        ):
            print(
                "L1 norm doesn't have a closed form solution when 1 < beta < 2. Ignore L1 regularization."
            )
            self._l1_reg_H = 0
            self._l1_reg_W = 0
        ...
        if self._beta != 2 and (self._l2_reg_H > 0 or self._l2_reg_W > 0):
            print(
                "L2 norm doesn't have a closed form solution when beta != 2. Ignore L2 regularization."
            )
            self._l2_reg_H = 0
            self._l2_reg_W = 0
    ...
    def _initialize_H_W(self, eps=1e-6):
        ...
        if self._init_method in ["nndsvd", "nndsvda", "nndsvdar"]:
            ...
            W = torch.zeros_like(
                V.T, dtype=self._tensor_dtype, device=self._device_type
            )
            H[:, 0] = S[0].sqrt() * U[:, 0]
            W[0, :] = S[0].sqrt() * V[:, 0]
            ...
            for j in range(2, self.k):
                x, y = U[:, j], V[:, j]
                x_p, y_p = x.maximum(
                    torch.zeros_like(x, device=self._device_type)
                ), y.maximum(torch.zeros_like(y, device=self._device_type))
                x_n, y_n = (
                    x.minimum(torch.zeros_like(x, device=self._device_type)).abs(),
                    y.minimum(torch.zeros_like(y, device=self._device_type)).abs(),
                )
    ...
    def fit(
        ...
        if torch.any(X < 0):
            raise ValueError(
                "The input matrix is not non-negative. NMF cannot be applied."
            )

        self.X = X
        if (
            self._beta == 2
        ):  # Cache sum of X^2 divided by 2 for speed-up of calculating beta loss.
