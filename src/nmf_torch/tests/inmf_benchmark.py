...
def loss(X, H, W, V, lam, dtype="double"):
    ...
    for k in range(len(X)):
        res += (
            torch.norm(
                X[k].double() - H[k].double() @ (W.double() + V[k].double()), p=2
            )
            ** 2
        )
        if lam > 0:
...
def run_test(
    mats, algo, mode, n_components, lam, n_jobs, seed, fp_precision, batch_max_iter
):
    print(f"{algo} {mode} Experiment...")

    ts_start = time.time()
    H, W, V, err = integrative_nmf(
        mats,
        algo=algo,
        mode=mode,
    ...
    print(
        f"{algo} {mode} finishes in {ts_end - ts_start} s, with error {err} (confirmed with {err_confirm})."
    )
