...
def main():
    ...
    method_output_dir = build_output_dir(
    ...
    for class_idx in pbar:
        ...
        if not args.patchify:
            ...
            out = ceh.select_class_and_load_images_v2(
                image_path_list=image_list,
                data_root=f"/scratch/{get_username()}/imagenet_data/{dataset}/",
            ...
            out = ceh.select_class_and_load_images_v2(
                image_path_list=image_group[class_idx],
                data_root=f"/scratch/{get_username()}/imagenet_data/{data_root_name}/",
        ...
        if concepts_0 is not None:
            ...
            act0_mean, act0_std = (
                comparison_dict["act1_mean"],
                comparison_dict["act1_std"],
            )
        ...
        if concepts_1 is not None:
            ...
            act1_mean, act1_std = (
                comparison_dict["act2_mean"],
                comparison_dict["act2_std"],
            )
                ...
                for key in ["l2", "kl", "match_acc", "dptc"]:
                    ...
                    if pred_U0 is not None:
                        for i in range(U0.shape[1]):
                            _scores[_key]["1to1"].append(
                                rep_out00[key].mean(1)[i].item()
                            )
                            _scores[_key]["2to1"].append(
                                rep_out0[key].mean(1)[i].item()
                            )

                    if pred_U1 is not None:
                        for i in range(U1.shape[1]):
                            _scores[_key]["2to2"].append(
                                rep_out11[key].mean(1)[i].item()
                            )
                            _scores[_key]["1to2"].append(
                                rep_out1[key].mean(1)[i].item()
                            )
