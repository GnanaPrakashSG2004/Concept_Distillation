...
def process_comparison_dict(
    ...
    if method == "pearson":
        ...
        method in ["lasso_regression", "lasso_regression_c", "k_nearest_neighbors"]
        and not use_train_data
    ):
    ...
    return dict(
        max_concept_match_rtoc=max_concept_match_rtoc,
        max_concept_match_ctor=max_concept_match_ctor,
    )


def visualize_similarity_vs_importance(
    class_list,
    comparison_dir,
    fe_outs,
    eval_dict,
    ...
    if visualize_each_class:
        class_specific_viz_folder_model0 = os.path.join(
            visualization_output_dir, "class_similarity_vs_importance", model0_name
        )
        class_specific_viz_folder_model1 = os.path.join(
            visualization_output_dir, "class_similarity_vs_importance", model1_name
        )
    ...
    if visualize_summary_plot_comparison:
        summary_plot_comparison_dir = os.path.join(
            visualization_output_dir, "summary_plot_comparison"
        )
    ...
    for ci in range(len(class_list)):
        ...
        if compute_weighted_int_grad:
            prob_delta = (
                eval_dict[model0_name]["sample"][class_idx]
                - eval_dict[model1_name]["sample"][class_idx]
            )
            ...
            _max = max(
                weighted_int_grad0[ci].mean(0).max(),
                weighted_int_grad1[ci].mean(0).max(),
            )
            _min = min(
                weighted_int_grad0[ci].mean(0).min(),
                weighted_int_grad1[ci].mean(0).min(),
            )
    ...
    for class_idx in pbar:
        ...
        max_concept_match_rtoc, max_concept_match_ctor = (
            out["max_concept_match_rtoc"],
            out["max_concept_match_ctor"],
        )
        ...
        if visualize_each_class:
            ...
            axes_r[0].scatter(
                max_concept_match_rtoc, int_grad0[ci].mean(0), alpha=0.5, color="blue"
            )
            [
                axes_r[0].annotate(
                    f"{i}", (max_concept_match_rtoc[i], int_grad0[ci].mean(0)[i])
                )
                for i in range(len(max_concept_match_ctor))
            ]
            axes_r[1].scatter(
                max_concept_match_rtoc,
                weighted_int_grad0[ci].mean(0),
                alpha=0.5,
                color="blue",
            )
            [
                axes_r[1].annotate(
                    f"{i}",
                    (max_concept_match_rtoc[i], weighted_int_grad0[ci].mean(0)[i]),
                )
                for i in range(len(max_concept_match_ctor))
            ]
            axes_r[0].set_xlim([-0.2, 1])
            f1, acc = (
                eval_dict[model0_name]["stats"][str(class_idx)]["f1"],
                eval_dict[model0_name]["stats"][str(class_idx)]["acc"],
            )
            plt.suptitle(
                f"{class_idx} | {model0_name} | F1: {f1:0.2f} | Acc: {acc:0.2f}"
            )
            plt.savefig(
                os.path.join(class_specific_viz_folder_model0, f"{class_idx}.png")
            )
            plt.close(fig_r)

            fig_c, axes_c = plt.subplots(2, 1)
            axes_c[0].scatter(
                max_concept_match_ctor, int_grad1[ci].mean(0), alpha=0.5, color="blue"
            )
            [
                axes_c[0].annotate(
                    f"{i}", (max_concept_match_ctor[i], int_grad1[ci].mean(0)[i])
                )
                for i in range(len(max_concept_match_ctor))
            ]
            axes_c[1].scatter(
                max_concept_match_ctor,
                weighted_int_grad1[ci].mean(0),
                alpha=0.5,
                color="blue",
            )
            [
                axes_c[1].annotate(
                    f"{i}",
                    (max_concept_match_ctor[i], weighted_int_grad1[ci].mean(0)[i]),
                )
                for i in range(len(max_concept_match_ctor))
            ]
            axes_c[0].set_xlim([-0.2, 1])
            f1, acc = (
                eval_dict[model1_name]["stats"][str(class_idx)]["f1"],
                eval_dict[model1_name]["stats"][str(class_idx)]["acc"],
            )
            plt.suptitle(
                f"{class_idx} | {model1_name} | F1: {f1:0.2f} | Acc: {acc:0.2f}"
            )
            plt.savefig(
                os.path.join(class_specific_viz_folder_model1, f"{class_idx}.png")
            )
            plt.close(fig_c)
        ...
        if visualize_summary_plot_comparison:
            ...
            for i in range(10):
                ...
                sum_axes[i, 0].set_xlabel(
                    f"Sim: {sim0[ind0]:0.2f} | IG: {ig0[ind0]:0.2f} | WIG: {wig0[ind0]:0.2f}"
                )
                sum_axes[i, 1].set_xlabel(
                    f"Sim: {sim1[ind1]:0.2f} | IG: {ig1[ind1]:0.2f} | WIG: {wig1[ind1]:0.2f}"
                )
            ...
            sum_axes[0, 0].set_title(
                f"{class_idx} | {model0_name} | F1: {m0_f1:0.2f} | Acc: {m0_acc:0.2f}"
            )
            sum_axes[0, 1].set_title(
                f"{class_idx} | {model1_name} | F1: {m1_f1:0.2f} | Acc: {m1_acc:0.2f}"
            )
            plt.tight_layout()
            # plt.show()
    ...
    ax = axes[0, 0]
    ax.hist(
        rtoc_arr, bins=np.linspace(-0.2, 1, 10), alpha=0.5, color="blue", label="rtoc"
    )
    ax.set_xlabel(
        f"{model1_plot_name}"
        + r"$\rightarrow$"
        + f"{model0_plot_name} Concept Similarity"
    )
    ax = axes[1, 0]
    ax.hist(
        ctor_arr, bins=np.linspace(-0.2, 1, 10), alpha=0.5, color="red", label="ctor"
    )
    ax.set_xlabel(
        f"{model1_plot_name}"
        + r"$\rightarrow$"
    ...
    cbar = fig.colorbar(
        im, ax=axes.ravel().tolist(), ticks=range(0, ceil(cmax), interval)
    )
    ...
    # plt.show()
    stats = {
        "mean_similarity": {"2to1": rtoc_arr.mean(), "1to2": ctor_arr.mean()},
        ...
        "weighted_mean_similarity": {
            "2to1": np.average(rtoc_arr, weights=ig0_arr),
            "1to2": np.average(ctor_arr, weights=ig1_arr),
        },
        "weighted_median_similarity": {
            "2to1": np.median(rtoc_arr * ig0_arr),
            "1to2": np.median(ctor_arr * ig1_arr),
        },
    }
    with open(
        os.path.join(visualization_output_dir, "similarity_stats.json"), "w"
    ) as f:
        json.dump(stats, f, indent=2)
    if save:
        plt.savefig(
            os.path.join(visualization_output_dir, "similarity_vs_importance.png"),
            dpi=600,
        )
        plt.savefig(
            os.path.join(visualization_output_dir, "similarity_vs_importance.pdf")
        )
    if show:
        plt.show()
    ...
    if compute_weighted_int_grad:
        ...
        if save:
            plt.savefig(
                os.path.join(
                    visualization_output_dir, "similarity_vs_weighted_importance.png"
                )
            )
        if show:
            plt.show()
    ...
    dist0 = np.linalg.norm(
        np.array([[0], [1]]) - np.stack([rtoc_arr, imp0], axis=0), axis=0
    )
    dist1 = np.linalg.norm(
        np.array([[0], [1]]) - np.stack([ctor_arr, imp1], axis=0), axis=0
    )
        ...
        samples0.append(
            {
                "class": class_idx,
                "concept": concept_idx,
                "similarity": rtoc_arr[ind].item(),
                "ci": ig0_arr[ind].item(),
            }
        )  # , 'wci': wig0_arr[ind].item()})
        ...
        samples1.append(
            {
                "class": class_idx,
                "concept": concept_idx,
                "similarity": ctor_arr[ind].item(),
                "ci": ig1_arr[ind].item(),
            }
        )  # , 'wci': wig1_arr[ind].item()})
    ...
    with open(
        os.path.join(
            visualization_output_dir, f"{output_name}_samples0_class_list.json"
        ),
        "w",
    ) as f:
    ...
    with open(
        os.path.join(
            visualization_output_dir, f"{output_name}_samples1_class_list.json"
        ),
        "w",
    ) as f:
    ...
    with open(
        os.path.join(visualization_output_dir, f"{output_name}_samples0.json"), "w"
    ) as f:
        json.dump(samples0, f, indent=2)
    with open(
        os.path.join(visualization_output_dir, f"{output_name}_samples1.json"), "w"
    ) as f:
    ...
    return dict(
        samples_class_lists=[s0_class_list, s1_class_list],
        rtoc=rtoc_arr,
        ctor=ctor_arr,
        ig0=int_grad0,
        ig1=int_grad1,
        wig0=weighted_int_grad0,
        wig1=weighted_int_grad1,
        rtoc_arr=rtoc_arr,
        ctor_arr=ctor_arr,
        m0_layer=m0_layer,
        m1_layer=m1_layer,
...
def load_model_stats(
    ...
    for mi, param_dicts in enumerate([param_dicts1, param_dicts2]):
        ...
        path = (
            f"model_evaluation/{dataset_name}/{model_name}_probs_{args.data_split}.pth"
        )
        print(path)
        probs = torch.load(path)
        with open(
            f"model_evaluation/{dataset_name}/{model_name}_{args.data_split}.json", "r"
        ) as f:
            eval_dict = json.load(f)
        with open(
            f"model_evaluation/{dataset_name}/{model_name}_stats_{args.data_split}.json",
            "r",
        ) as f:
            stats = json.load(f)
...
def compare_similarity_methods(
    ...
    for pair in pairs:
        ...
        if save:
            plt.savefig(
                os.path.join(out_folder, f"{method1_name}_{method2_name}.png"), dpi=400
            )
        if show:
            plt.show()
...
def main():
    ...
    parser.add_argument(
        "--importance_output_root", type=str, default="/scratch/swayam/rsvc-exps"
    )
    ...
    for mi, method_dict in enumerate(comparison_methods):
        ...
        visualization_output_dir = os.path.join(
            ...
            "visualizations",
            (
                "similarity_vs_importance"
                if not args.visualize_baseline
                else "similarity_vs_importance_baseline"
            ),
            comparison_folder,
        )
        ...
        out = visualize_similarity_vs_importance(
            class_list,
            comparison_dir,
            fe_outs,
            eval_dict,
    ...
    compare_similarity_methods(
        method_max_concept_sim,
        output_dir,
        ph.plot_names[model0_name],
        ph.plot_names[model1_name],
        show=False,
    )
