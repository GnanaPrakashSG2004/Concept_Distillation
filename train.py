...
class ConceptDistillationTrainer:
    def __init__(
        ...
        self.metrics = {"epoch_losses": [], "batch_losses": []}
    ...
    def extract_features_from_last_layer(self, model, x):
        ...
        if hasattr(model, "layer4"):  # ResNet-like architecture
            ...
            hasattr(model, "blocks") and len(model.blocks) > 0
        ):  # ViT-like architecture
            ...
            self.logger.warning(
                "Could not identify target layer for hooks, using fallback method"
            )
    ...
    def compute_projection_matrix(self, student_features, teacher_concepts, lambda_reg):
        ...
        teacher_concepts = torch.tensor(
            teacher_concepts, dtype=torch.float32, device=self.device
        )
    ...
    def find_nmf_fixed_H(
        ...
        if np.any(teacher_features_np < 0):
            if verbose:
                warnings.warn(
                    "Input 'teacher_features' contains negative values. "
                    "Clipping to 0 for NMF.",
                    UserWarning,
                )
            teacher_features_np = np.maximum(
                teacher_features_np, 0
        ...
        if np.any(concept_bank < 0):
            if verbose:
                warnings.warn(
                    "Input 'W_teacher' contains negative values. "
                    "Clipping to 0 for NMF.",
                    UserWarning,
                )
        ...
        W_teacher_nonneg = W_teacher_nonneg.astype(
            teacher_features_np.dtype, copy=False
        )
    ...
    def concept_alignment_loss(self, student_features, W_star, teacher_concept_coeffs):
        ...
        alignment_loss = torch.mean(
            torch.square(student_concepts - teacher_concept_coeffs)
        )
    ...
    def compute_projection_matrix_nmf(
        ...
        if isinstance(student_features, torch.Tensor):
            if verbose:
                print("Converting student_features to NumPy.")
        ...
        if np.any(U < 0):
            ...
            warnings.warn(
                "Input 'target_coefficients' (U) contains negative values. "
                "Clipping to 0 for NMF.",
                UserWarning,
            )
            U = np.maximum(U, 0)
        ...
        if A_S.shape[0] != U.shape[0]:
            raise ValueError(
                f"Incompatible shapes: student_features has {A_S.shape[0]} samples, "
                f"but target_coefficients has {U.shape[0]} samples."
            )

        if lambda_reg < 0:
            raise ValueError(
                "lambda_reg (regularization strength) must be non-negative."
            )
        ...
        if verbose:
            print(
                f"Setting up NMF to find W* ({n_features}, {k_targets}) solving U ≈ A_S @ W*"
            )
        ...
        if verbose:
            print(
                f"Running NMF with fixed W (A_S), finding H (W*) shape {H_init.shape}..."
            )
        ...
        try:
            with warnings.catch_warnings():
                ...
                warnings.filterwarnings(
                    "ignore", message=".*init='custom'.*", category=FutureWarning
                )
                nmf_model.fit(
                    X=U, W=A_S, H=H_init
                )  # Pass fixed W=A_S, initial H=H_init
        ...
        if verbose and np.any(W_star < 0):
            ...
            print(
                "Warning: NMF resulted in negative values in W* (unexpected). Clipping."
            )
    ...
    def train(self, epochs=1, lr=0.001):
        ...
        for epoch in range(epochs):
            ...
            while valid_class_ids:
                ...
                U_teacher = self.find_nmf_fixed_H(
                    teacher_features, W_teacher, verbose=False
                )
                ...
                W_star = self.compute_projection_matrix_nmf(
                    student_features, U_teacher, verbose=False
                )
                """
            ...
            self.logger.info(
                f"Epoch {epoch+1} completed in {epoch_time:.2f}s, Avg Loss: {avg_epoch_loss:.4f}"
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--save-dir",
        type=str,
        default="/scratch/swayam/rsvc_model_trains",
    ...
    parser.add_argument(
        "--log-dir",
        type=str,
        default="/scratch/swayam/rsvc_logs",
...
if __name__ == "__main__":
    ...
    if args.use_wandb:
        if WANDB_AVAILABLE:
            ...
            wandb.init(
                ...
                or f"distill_{args.teacher_model}_to_{args.student_model}",
                config=vars(args),
            )
        else:
    ...
    student_model = (
        timm.create_model(
            args.student_model, pretrained=True, cache_dir=args.timm_cache
        )
        .train()
        .to(args.device)
    )
    ...
    base_dataset = datasets.ImageNet(
        root=args.data_root, split=args.split, transform=transform
    )
