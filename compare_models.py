...
def regression_comparison(method_name, params, activations1, activations2, U1, U2):
    ...
    for k, (train_indices, test_indices) in enumerate(splits):
        ...
        if U1 is not None:
            ...
            # pearson correlation
            lr1to1_pearson = np.array(
                [
                    pearsonr(test_U1[:, i], pred_A1U1[:, i]).statistic
                    for i in range(U1.shape[1])
                ]
            )
            lr2to1_pearson = np.array(
                [
                    pearsonr(test_U1[:, i], pred_A2U1[:, i]).statistic
                    for i in range(U1.shape[1])
                ]
            )
            # score
        ...
        if U2 is not None:
            ...
            # pearson correlation
            lr1to2_pearson = np.array(
                [
                    pearsonr(test_U2[:, i], pred_A1U2[:, i]).statistic
                    for i in range(U2.shape[1])
                ]
            )
            lr2to2_pearson = np.array(
                [
                    pearsonr(test_U2[:, i], pred_A2U2[:, i]).statistic
                    for i in range(U2.shape[1])
                ]
            )
            # score
    ...
    for mei, method_dict in enumerate(methods_list):
        ...
        if method in ["pearson", "spearman"]:
            ...
            if U1 is not None or U2 is not None:
                out = regression_comparison(
                    method, method_dict, activations1, activations2, U1, U2
                )
            else:
                out = None
        else:
...
def process_config(config, outdir):
    ...
    assert (train_pct is None) or (
        num_folds is None
    ), "Cannot specify both train_pct and num_folds"
    assert (train_pct is not None) or (
        num_folds is not None
    ), "Must specify either train_pct or num_folds"
...
def build_model_comparison_parser():
    ...
    parser.add_argument(
        "--comparison_output_root", type=str, default="/scratch/swayam/rsvc-exps/"
    )
    return parser
...
def k_fold_split_image_group(
    image_list, num_folds, num_image_repeats, patched=False, num_patches=None
):
    ...
    if not patched:
        ...
        test_image_indices = {}
        for k, (train_idx, test_idx) in enumerate(splits):
            train_image_indices[k] = np.concatenate(
                [index_map[ind] for ind in splits[k][0]]
            ).tolist()
            test_image_indices[k] = np.concatenate(
                [index_map[ind] for ind in splits[k][1]]
            ).tolist()
        # print()
        # plt.subplots(5, 1, figsize=(20, 5))
        # for i in range(5):
...
def shared_concept_proposals_inference(params):
    ...
    if patchify:
        ...
        if dataset == "imagenet":
            ...
            ) * np.ceil(transforms[0].transforms[1].size[0] / patch_size)
            ...
            ) * np.ceil(transforms[0].transforms[0].size[0] / patch_size)
        ...
        image_list, train_indices, test_indices = k_fold_split_image_group(
            image_group[class_idx], num_folds, 1, patched=True, num_patches=num_patches
        )

    else:
        if len(image_group[class_idx]) < num_images:
            print(
                f"Scaling number of repeats to match number of available images -- {class_idx}"
            )
            num_image_repeats = target_num_samples // len(image_group[class_idx])

        image_list, train_indices, test_indices = k_fold_split_image_group(
            image_group[class_idx], num_folds, num_image_repeats
        )
...
def main():
    ...
    for mi, param_dicts in enumerate([param_dicts1, param_dicts2]):
        ...
        transform = (
            ...
            if transform_type == "test" or patchify
    ...
    for class_idx in pbar:
        ...
        params = dict(
            ...
            num_image_repeats=num_image_repeats,
            num_folds=num_folds,
            regression_train_pct=regression_train_pct,
            dataset=dataset,
            device=device,
            class_idx=class_idx,
            fe_outs=fe_outs,
