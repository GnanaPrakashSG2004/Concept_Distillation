...
def concept_integrated_gradients(
    u, w, head, class_idx, steps=30, device="cuda", activations=None
):
    """
...
def build_output_dir(args, config, save_names1, save_names2, data_group_name=None):
    ...
    output_dir = saving.build_output_dir(
        args.importance_output_root, "concept_importance", comparison_name
    )
    ...
    model1_out_dir = os.path.join(
        output_dir, data_group_name, save_names1["model_name"]
    )
    model2_out_dir = os.path.join(
        output_dir, data_group_name, save_names2["model_name"]
    )
...
def main():
    ...
    model1_out_dir, model2_out_dir = build_output_dir(
        args, config, save_names1, save_names2
    )
    ...
    for class_idx in pbar:
        ...
        if not args.patchify:
            ...
            images_preprocessed = ceh.patchify_images(
        ...
        for mi in range(len(fe_outs)):
            ...
            integrated_gradients = concept_integrated_gradients(
                U.cuda(),
                W.cuda(),
                clfs[mi],
                class_idx,
            ...
            torch.save(
                integrated_gradients.cpu(), os.path.join(output_dir, f"{class_idx}.pth")
            )
