...
def _select_class_and_load_images(
    image_path_list, data_root, crop_and_resize_fn, transform, repeat=1
):
    ...
    for i, img_path in enumerate(sel_paths):
        ...
        cropped_images.append(
            torchvision.transforms.ToTensor()(
                torchvision.transforms.Resize((64, 64))(crop_img)
            )
        )
...
def measure_pred_matrix_effect(clf, Ub_pred, Ub, Wb, class_idx):
    ...
    l2_dist = torch.norm(Xb_pred_rec - Xb_rec, dim=-1, p=2).cpu().type(torch.float16)
    ...
    delta_prob_target_class = (
        Yb_replaced_probs[:, class_idx] - Yb_targ_probs[:, class_idx]
    )
    kl_div = (
        F.kl_div(
            Yb_replaced_logprobs, Yb_targ_logprobs, reduction="none", log_target=True
        )
        .nansum(-1)
        .cpu()
        .type(torch.float16)
    ...
    return dict(
        ...
        kl=kl_div,
        match_acc=matches,
        dptc=delta_prob_target_class,
        replaced_probs=Yb_replaced_probs.cpu(),
        target_probs=Yb_targ_probs.cpu(),
    )
...
def run_replacement_test(clf, Ub_pred, Ub, Wb, class_idx):
    ...
    l2_dist = (
        torch.norm(
            Xb_replaced - Xb_rec.unsqueeze(0).repeat(Ub_num_concepts, 1, 1), dim=-1, p=2
        )
        .cpu()
        .type(torch.float16)
    )
    ...
    Yb_replaced_preds = clf(Xb_replaced.view(-1, Xb_replaced.shape[-1])).view(
        Xb_replaced.shape[0], Xb_replaced.shape[1], -1
    )
    Yb_replaced_logprobs = torch.log_softmax(Yb_replaced_preds, dim=-1)
    Yb_replaced_probs = torch.softmax(Yb_replaced_preds, dim=-1)
    delta_prob_target_class = (
    ...
    kl_div = (
        F.kl_div(
            Yb_replaced_logprobs.view(-1, num_classes),
            Yb_targ_logprobs.unsqueeze(0)
            .repeat(Ub_num_concepts, 1, 1)
            .view(-1, num_classes),
            reduction="none",
            log_target=True,
        )
        .nansum(-1)
        .reshape(Ub_num_concepts, -1)
        .cpu()
        .type(torch.float16)
    )
    matches = Yb_replaced_preds.argmax(-1) == Yb_targ_probs.argmax(-1)
...
def visualize_concept_differences(
    args,
    plot_params,
    show=True,
    save=False,
    sim_pair_output_folder="concept_differences",
):
    ...
    viz_folder = os.path.join(
        args.visualization_output_root,
        "outputs",
        "visualizations",
        f"{sim_pair_output_folder}",
        folder,
    )
    ...
    for i in concept_list:
        ...
        ax.scatter(
            U[top_d_max_inds, axi],
            pred_U_unnorm[top_d_max_inds, axi],
            color=over_color,
            s=26,
        )
        ax.scatter(
            U[top_d_min_inds, axi],
            pred_U_unnorm[top_d_min_inds, axi],
            color=under_color,
            s=26,
        )
        ax.scatter(
            U[topk, axi],
            pred_U_unnorm[topk, axi],
            color=concept_color,
            s=26,
        )
        ...
        if save:
            plt.savefig(
                os.path.join(viz_folder, f"{direction}_{class_idx}-{i}_scatter.png"),
                dpi=400,
            )
        ...
        if save:
            plt.savefig(
                os.path.join(viz_folder, f"{direction}_{class_idx}-{i}_over.png"),
                dpi=400,
            )
        if show:
            plt.show()
        ...
        sel_images = (
            image_samples[top_d_min_inds[:10]].permute(0, 2, 3, 1).cpu().numpy()
        )
        fig = make_image_grid(sel_images)
        fig.patch.set_linewidth(linewidth)
        ...
        if save:
            plt.savefig(
                os.path.join(viz_folder, f"{direction}_{class_idx}-{i}_under.png"),
                dpi=400,
            )
        if show:
            plt.show()
        ...
        if save:
            plt.savefig(
                os.path.join(viz_folder, f"{direction}_{class_idx}-{i}_topk.png"),
                dpi=400,
            )
        if show:
            plt.show()
...
def main():
    ...
    parser.add_argument(
        "--visualization_output_root", type=str, default="/scratch/swayam/rsvc-exps"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sim_pair_output_folder", type=str, default="paper_qualitative"
    )
    parser.add_argument(
        "--selected_samples_json", type=str, default=None, required=True
    )
    ...
    for mi, param_dicts in enumerate([param_dicts1, param_dicts2]):
        ...
        transform = (
            model_out["test_transform"]
            if transform_type == "test"
            else model_out["transform"]
        )
        ...
        stats = eval_model.load_or_run_evaluation(
    ...
    act_hook1.register_hooks(
        fe_outs[0]["layer_names"][-1:],
        fe_outs[0]["layers"][-1:],
        fe_outs[0]["post_activation_func"],
    )

    act_hook2 = ActivationHook()
    act_hook2.register_hooks(
        fe_outs[1]["layer_names"][-1:],
        fe_outs[1]["layers"][-1:],
    ...
    unnormalized_test_transform = torchvision.transforms.Compose(
        model_out["test_transform"].transforms[:-1]
    )
    ...
    image_group, eval_dicts = create_image_group(
        strategy=igs, param_dicts=[param_dicts1, param_dicts2], return_eval_dict=True
    )
    ...
    for method_dict in comparison_methods:
        if (
            method_dict["method"] == "lasso_regression"
            or method_dict["method"] == "lasso_regression_c"
        ):
            break
    ...
    for class_idx in pbar:
        ...
        try:
            comparison_file = os.path.join(
                ...
                f"{class_idx}",
                f"{layer_name_0}-{layer_name_1}.pkl",
            )
        ...
        m0_pred_recon = (
            clfs[0](torch.FloatTensor((U0 @ W0)).cuda()).argmax(-1).cpu().numpy()
        )
        m1_pred_recon = (
            clfs[1](torch.FloatTensor((U1 @ W1)).cuda()).argmax(-1).cpu().numpy()
        )
        ...
        activations0_norm, _, _ = standardize(
            activations0, mean=act0_mean, std=act0_std
        )
        activations1_norm, _, _ = standardize(
            activations1, mean=act1_mean, std=act1_std
        )
        ...
        if type(comparison_dict["lr1to1"]) == list:
            ...
            for lr_ind in range(len(comparison_dict["lr1to1"])):
                pred_U1.append(
                    comparison_dict["lr1to2"][lr_ind].predict(activations0_norm)
                )
                pred_U0.append(
                    comparison_dict["lr2to1"][lr_ind].predict(activations1_norm)
                )
                pred_U0U0.append(
                    comparison_dict["lr1to1"][lr_ind].predict(activations0_norm)
                )
                pred_U1U1.append(
                    comparison_dict["lr2to2"][lr_ind].predict(activations1_norm)
                )
        ...
        for func_tuple in [
            ...
            if func_name == "replacement":
                ...
                m0_pred_recon = (
                    clfs[0](torch.FloatTensor((pred_U0_unnorm @ W0)).cuda())
                    .argmax(-1)
                    .cpu()
                    .numpy()
                )
                m1_pred_recon = (
                    clfs[1](torch.FloatTensor((pred_U1_unnorm @ W1)).cuda())
                    .argmax(-1)
                    .cpu()
                    .numpy()
                )
                ...
                prec_0_regr_recon, rec_0_regr_recon = comp_prec_recall(
                    gt_mask, m0_pred_recon == class_idx
                )
                prec_1_regr_recon, rec_1_regr_recon = comp_prec_recall(
                    gt_mask, m1_pred_recon == class_idx
                )
                print(
                    "Prec 1->0 recon: ",
                    prec_0_regr_recon,
                ...
                rep_out0 = run_replacement_test(
                    clfs[0], _tt(pred_U0_unnorm), _tt(U0), _tt(W0), class_idx
                )
                rep_out1 = run_replacement_test(
                    clfs[1], _tt(pred_U1_unnorm), _tt(U1), _tt(W1), class_idx
                )
                rep_out00 = run_replacement_test(
                    clfs[0], _tt(pred_U0U0_unnorm), _tt(U0), _tt(W0), class_idx
                )
                rep_out11 = run_replacement_test(
                    clfs[1], _tt(pred_U1U1_unnorm), _tt(U1), _tt(W1), class_idx
                )
                _scores = {}
                ...
                rep_out0 = measure_pred_matrix_effect(
                    clfs[0], _tt(pred_U0_unnorm), _tt(U0), _tt(W0), class_idx
                )
                rep_out1 = measure_pred_matrix_effect(
                    clfs[1], _tt(pred_U1_unnorm), _tt(U1), _tt(W1), class_idx
                )
                rep_out00 = measure_pred_matrix_effect(
                    clfs[0], _tt(pred_U0U0_unnorm), _tt(U0), _tt(W0), class_idx
                )
                rep_out11 = measure_pred_matrix_effect(
                    clfs[1], _tt(pred_U1U1_unnorm), _tt(U1), _tt(W1), class_idx
                )
                _scores = {}
                for key in ["l2", "kl", "match_acc", "dptc"]:
                ...
                for i in range(U0.shape[1]):
                    ...
                    _scores["1to1"].append(
                        func(U0[:, i], pred_U0U0_unnorm[:, i]).statistic
                    )
                    _scores["2to2"].append(
                        func(U1[:, i], pred_U1U1_unnorm[:, i]).statistic
                    )
                    _scores["2to1"].append(
                        func(U0[:, i], pred_U0_unnorm[:, i]).statistic
                    )
                    _scores["1to2"].append(
                        func(U1[:, i], pred_U1_unnorm[:, i]).statistic
                    )
                scores[func_name] = _scores

        comparison_output_file = comparison_file.replace(
            "concept_comparison", output_folder
        )
        ...
        if args.visualize_concepts:
            ...
            if class_list_values[ci] <= 0:
                ...
                visualize_concept_differences(
                    args,
                    plot_params,
                    save=True,
                    show=False,
                    sim_pair_output_folder=args.sim_pair_output_folder,
                )
            if class_list_values[ci] >= 0:
                plot_params = {
                    "class_idx": class_idx,
